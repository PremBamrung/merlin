...
(schema built by the real Alembic migration `005`), plus a continuation check
...
def test_title_generated_on_first_save(client, monkeypatch):
    from merlin.services import chat_history

    monkeypatch.setattr(chat_history, "generate_title", lambda _t: "Moat Strategies")
    chat_history.save_thread("t1", [_user("tell me about moats")])
    assert chat_history.get_thread("t1")["title"] == "Moat Strategies"

    # A later save must NOT regenerate (title already set) — patch to a sentinel
    # that would fail the assertion if called.
    monkeypatch.setattr(chat_history, "generate_title", lambda _t: "SHOULD-NOT-BE-USED")
    chat_history.save_thread("t1", [_user("tell me about moats"), _assistant("...")])
    assert chat_history.get_thread("t1")["title"] == "Moat Strategies"
...
def test_endpoints_put_get_list_delete(client, monkeypatch):
    ...
    assert r.json() == {"id": "t1", "title": "Titled"}
