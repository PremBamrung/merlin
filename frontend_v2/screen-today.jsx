/* global React, I, Sidebar, Topbar, SourcePill, VIDEOS, BLOGS, REDDITS, ALL_SOURCES, TAGS */
const { useState: useStateToday } = React;

function TodayScreen() {
  const [input, setInput] = useStateToday('');
  const [focused, setFocused] = useStateToday(false);
  const [queue, setQueue] = useStateToday([
          ...
          <div className="page-narrow">
            <div style={{ marginBottom: 28 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 4 }}>
                <span className="mono" style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase' }}>Wednesday, April 22</span>
            ...
            <div className={"omnibox" + (focused ? ' is-focused' : '')}>
              <div className="omnibox-input">
                <I.sparkle style={{ width: 18, height: 18, color: 'var(--accent)', flexShrink: 0 }} />
                <input
              ...
              <div className="omnibox-hint">
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: 10, alignItems: 'center' }}>
                  ...
                  <SourcePill type="youtube" /> <SourcePill type="blog" /> <SourcePill type="reddit" /> <SourcePill type="web" />
                </div>
              </div>
            </div>

            {/* Queue — active ingestion */}
            {queue.length > 0 && (
              <div style={{ marginTop: 28 }}>
                ...
                <div className="grid-gap-8" style={{ gap: 8 }}>
                  {queue.map((q, i) => (
            ...
            <div style={{ marginTop: 36 }}>
              <div className="section-head">
                ...
                <span className="text-subtle" style={{ fontSize: 12, cursor: 'pointer' }}>View library →</span>
              </div>
              <div className="grid-gap-12" style={{ gap: 10 }}>
                {ALL_SOURCES.slice(0, 4).map((s) => (
                  <div key={s.id} className="recent-row">
                    <div className="recent-thumb">
                      {s.type === 'youtube' && <I.yt style={{ width: 20, height: 20 }} />}
                      {s.type === 'blog' && <I.paper style={{ width: 20, height: 20 }} />}
                      {s.type === 'reddit' && <I.reddit style={{ width: 20, height: 20 }} />}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 3 }}>
                        <SourcePill type={s.type} />
                        <span className="text-subtle" style={{ fontSize: 11 }}>{s.channel || s.author || s.subreddit}</span>
                        <span className="text-subtle" style={{ fontSize: 11 }}>·</span>
                      ...
                      <div style={{ fontSize: 12.5, color: 'var(--text-muted)', lineHeight: 1.45, display: '-webkit-box', WebkitBoxOrient: 'vertical', WebkitLineClamp: 2, overflow: 'hidden' }}>
                        {s.summary}
                      </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  ...
                  <div key={i} className="prompt-chip">
                    <I.chat style={{ width: 14, height: 14, color: 'var(--accent)', flexShrink: 0 }} />
                    <span>{q}</span>
                  </div>
                ))}
              </div>
